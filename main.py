...
CATEGORY_URLS = {
    ...
    "ギミック": "https://booth.pm/ja/search/VRChat?category=3d_tool_system&sort=new",
}
...
class MyBot(commands.Bot):
    ...
    async def init_db(self):
        async with aiosqlite.connect("bot_data.db") as db:
            ...
            try:
                await db.execute("ALTER TABLE tracked_items ADD COLUMN image_url TEXT")
            except Exception:
                pass  # すでにカラムが存在する場合はスルー
...
class CategorySelectView(discord.ui.View):
    ...
    async def select_callback(self, interaction: discord.Interaction):
        ...
        print(f"📌 【チャンネル登録完了】Channel ID: {channel_id} に {categories_display} を保存しました")

@bot.tree.command(name="set-channel", description="このチャンネルにBOOTH新作通知を設定します")
@app_commands.checks.has_permissions(manage_channels=True)
async def set_channel(interaction: discord.Interaction):
    view = CategorySelectView()
    await interaction.response.send_message("通知を受け取りたいジャンルを選んでね：", view=view, ephemeral=True)

@set_channel.error
async def set_channel_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    if isinstance(error, app_commands.MissingPermissions):
        await interaction.response.send_message("⚠️ このコマンドを使うには「チャンネルの管理」権限が必要です！", ephemeral=True)
...
async def remove_channel(interaction: discord.Interaction):
    async with aiosqlite.connect("bot_data.db") as db:
        ...
        await db.commit()
    await interaction.response.send_message("❌ このチャンネルの通知設定を解除したよ。", ephemeral=True)
            ...
            try:
                async with session.get(url, timeout=aiohttp.ClientTimeout(total=15)) as response:
                    if response.status != 200:
                        print(f"⚠️ [{cat_name}] アクセス失敗（ステータスコード: {response.status}）")
                        continue
                    html = await response.text()

                soup = bs4.BeautifulSoup(html, "html.parser")
                
                items = soup.select("li.item-card") or soup.select(".item-card") or soup.select("ul.grid > li")
                ...
                async with aiosqlite.connect("bot_data.db") as db:
                    for item in items:
                        try:
                            # タイトル・URLの取得
                            url_tag = item.select_one("a.item-card__title-anchor") or item.select_one(".item-card__title a") or item.select_one("a[href*='/items/']")
                            ...
                            # サムネイル画像の取得
                            img_tag = item.select_one(".item-card__thumbnail-images img") or item.select_one("img.js-thumbnail") or item.select_one("img")
                            image_url = ""
                            if img_tag:
                                image_url = img_tag.get("data-original") or img_tag.get("data-src") or img_tag.get("src") or ""

                            # 価格の取得
                            price_tag = item.select_one(".price") or item.select_one(".item-card__price")
                            price = price_tag.text.strip() if price_tag else "不明"

                            # スキ数の取得
                            like_tag = item.select_one(".js-like-count") or item.select_one(".item-card__like-count")
                            likes = 0
                            if like_tag:
                            ...
                            """, (item_id, title, item_url, price, cat_name, likes, image_url, now))
                            await db.commit()

                            # 送信チェック（未送信のアイテムのみ）
                            async with db.execute("SELECT notified FROM tracked_items WHERE item_id = ?", (item_id,)) as cursor:
                                row = await cursor.fetchone()
                                is_notified = row[0] if row else 0

                            if is_notified == 0:
                                success = await broadcast_item(item_id, title, item_url, price, category=cat_name, likes=likes, image_url=image_url, db=db)
                                if success:
                                    await db.execute("UPDATE tracked_items SET notified = 1 WHERE item_id = ?", (item_id,))
                                    await db.commit()
                ...
                print(f"✅ [{cat_name}] 解析完了: {parsed_count} / {len(items)} 件")

            except Exception as e:
                print(f"❌ [{cat_name}] 巡回通信エラー: {e}")

async def broadcast_item(item_id, title, url, price, category, likes, image_url, db):
    async with db.execute("SELECT channel_id, categories FROM channels") as cursor:
        ...
        if category in cat_list:
            ...
            if channel:
                try:
                    ...
                    print(f"🚀 【送信成功】#{channel.name} に 「{title[:15]}...」 を通知しました！")
                    any_success = True
